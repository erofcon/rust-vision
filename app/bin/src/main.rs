...
// Переключаемся на Actix-runtime:
#[actix_web::main]
async fn main() -> Result<()> {
    ...
    // 3. Спауним воркеры в фоне через actix_rt::spawn
    ...
    for id in 0..worker_cfg.worker_count {
        ...
        actix_web::rt::spawn(async move {
            // handler::spawn_worker возвращает JoinHandle<()>
            let _ = handler::spawn_worker(id, chan, repo, model).await;
        });
    }

    // 4. Запуск HTTP-сервера – здесь .run() возвращает локальное (не-Send) будущее
    ...
    .bind(&bind_addr)?
    .run()
    .await?; // <-- здесь мы просто .await, без tokio::spawn
