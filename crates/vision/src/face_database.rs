use crate::detection::{Detectors, FaceModels, Inference, Output, Prepare};
use anyhow::Result;
...
impl FaceDatabase {
    ...
    pub async fn load_from_folder(&mut self) -> Result<()> {
        let config = ProjectConfig::load()?;
        let base_dir = config.face_database.path;

        let face_models = FaceModels::load_from_file(
            &config.face_detection_model.path,
        ...
        for entry in fs::read_dir(base_dir)? {
            ...
            for img_e in fs::read_dir(&dir)? {
                ...
                let face_recognition_model = face_models.recognition.read();

                let inference =
                    Detectors::yolo11_inference(detection_model.get_session(), prepare)?;
                ...
                let emb = Detectors::extract_face_embedding(
                    face_recognition_model.get_session(),
                    prepare_rec,
                )?;
